...
class RBEDrivetrain:
	...
	def spinAboutWheel(self, speed, deg, wheel, pause):
		if wheel == "LEFT":
			self.motorRight.spin_for(REVERSE, self.finalDrive * deg * self.wheelRotDegPerBodyTurnDeg, DEGREES, speed, RPM, pause)
		elif wheel == "RIGHT":
			self.motorLeft.spin_for(REVERSE, self.finalDrive * deg * self.wheelRotDegPerBodyTurnDeg, DEGREES, speed, RPM, pause)

	def spinInPlace(self, speed, deg, pause):
		self.motorLeft.spin_for(FORWARD, self.finalDrive * deg / 2 * self.wheelRotDegPerBodyTurnDeg, DEGREES, speed, RPM, False)
		self.motorRight.spin_for(FORWARD, - self.finalDrive * deg / 2 * self.wheelRotDegPerBodyTurnDeg, DEGREES, speed, RPM, pause)
...
rbeDriveTrain = RBEDrivetrain(rangeFinderFront, rangeFinderRight, leftMotor, rightMotor, finalDrive, wheelDiameter, wheelBase, wheelTrack)
...
rbeDriveTrain.spinAboutWheel(100, 90, "RIGHT", True)
rbeDriveTrain.spinAboutWheel(100, -90, "RIGHT", True)

rbeDriveTrain.spinInPlace(100, 90, True)
